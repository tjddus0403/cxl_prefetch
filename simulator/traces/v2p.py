...
def v2p(page_table, va_trace):
    ...
    # trace_df.to_csv("astar_MB.csv", index=False)
    return trace_df

vaddr_list = ['sphinx3','sssp']

for vaddr in vaddr_list:
    vaddr_file = "./vaddr/"+vaddr+"_5m.vaddr"

    va_df = pd.read_csv(vaddr_file, names=['va'])
    va_trace = va_df['va']
    ...
    ## v2p
    paddr_file = "./paddr/2MB/"+vaddr+"/"+vaddr+"_5m.paddr"
    v2p_result['pa'].to_csv(paddr_file, index=False)
